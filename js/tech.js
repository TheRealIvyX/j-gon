const tech = {
    ...
    duplicationChance() {
        return Math.max(0, (tech.isTreasure ? 0.35 : 0) + (tech.isPowerUpsVanish ? 0.12 : 0) + (tech.isStimulatedEmission ? 0.15 : 0) + tech.cancelCount * 0.043 + tech.duplicateChance + m.duplicateChance + tech.fieldDuplicate + tech.cloakDuplication + (tech.isAnthropicTech && tech.isDeathAvoidedThisLevel ? 0.5 : 0))
    },
        ...
        },
        {
            ...
            effect: () => {
                ...
                m.regenEnergy = function() {
                    ...
                    m.energy += m.fieldRegen*4
                    if (m.energy < 0) m.energy = 0
                }
		for (let i = 0; i < 6; i++) powerUps.spawn(m.pos.x + Math.random() * 60, m.pos.y + Math.random() * 60, "research", false)
            },
            remove() {
