...
const level = {
    ...
    levelAnnounce() {
        ...
        if (level.levelsCleared === 0) {
            document.title = "j-gon: (" + difficulty + ")";
        } else {
            document.title = `j-gon: ${level.levelsCleared} ${level.levels[level.onLevel]} (${difficulty})`
    ...
    populateLevels() { //run a second time if URL is loaded
        ...
        if (simulation.isTraining) {
            ...
            if (simulation.isCommunityMaps) {
                ...
                level.levels = level.levels.concat(level.jgonLevels)
                ...
                level.levels = level.levels.concat(level.jgonLevels)
                level.levels = shuffle(level.levels); //shuffles order of maps
	        level.levels.splice(0, level.jgonLevels.length)
            }
